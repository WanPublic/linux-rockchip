 ...
 * V0.0X01.0X09
 * 2026-01-26: Replaced with orange-pi-5-plus-imx415-4k-90fps-singleisp branch driver to debug sync issues - Antigravity
 */
...
static const struct regval imx415_null_regs[] = {
	{REG_NULL, 0x00},
};
...
static __maybe_unused const struct regval imx415_linear_10bit_4k90fps_2376M_regs[] = {
	...
	{0x30C1, 0x00},	// XVS_DRV[1:0] - XVS output drive capability
	{0x30D9, 0x06},
	...
	// ===== 帧时序参数 =====
	{0x3024, 0xCE}, // VMAX[7:0] - Vertical max period (frame length in lines)
	{0x3025, 0x08}, // VMAX[15:8]
	{0x3026, 0x00}, // VMAX[19:16]
	                // ISM: 90fps VMAX=8CEh (2254 lines)
	                // 完整值: 0x08CE = 2254 lines
...
static const struct imx415_mode supported_modes[] = {
	{
		...
		/* VTS = 2254, 90fps, HTS = 4696 */
		.exp_def = 0x08CE - 0x08,
		.hts_def = 0x024B * IMX415_4LANES * 2,
		.vts_def = 0x08CE,
		.global_reg_list = imx415_null_regs,
		.reg_list = imx415_linear_10bit_4k90fps_2376M_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 4,
		.bpp = 10,
		.vc[PAD0] = 0,
				...
				struct v4l2_subdev_selection *sel)
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		/* 2026-01-18: 返回当前模式的完整分辨率作为裁剪边界。
		 * 以前硬编码为 3840x2160 导致 RKCIF 在 4K 模式下过早裁剪图像，
		 * 这在 90fps 模式下会导致尺寸不匹配。
		 */
		sel->r.left = 0;
