 ...
 * V0.0X01.0X09
 * 1. add 1920x1080 @ 90fps mode as default (2/2-line binning)
 * 2. based on register settings from sunnic_IMX415_RegisterSetting_Ver10.0_20240925_No1.ism
 * 3. uses 1782Mbps MIPI rate, 12bit output, 37.125MHz xvclk
 * 4. added crop support for 1920x1080 and 1932x1096 resolutions
 * 2026-01-27 by WanPublic
 */
...
/*
 * IMX415-AAQR 2/2-line binning CSI-2_4lane 37.125MHz AD:10bit Output:12bit 1782Mbps Master Mode 90.011fps
 * 1920x1080 @ 90fps
 * Register settings from sunnic_IMX415_RegisterSetting_Ver10.0_20240925_No1.ism
 */
static __maybe_unused const struct regval imx415_linear_12bit_1920x1080_90fps_1782M_regs[] = {
	{0x3020, 0x01},  // HADD
	{0x3021, 0x01},  // VADD
	...
	{0x3BC8, 0xBD},
	{0x3BCA, 0xBD},
	{0x4004, 0x48},  // TXCLKESC_FREQ[15:0] low byte
	{0x4005, 0x09},  // TXCLKESC_FREQ[15:0] high byte
	{0x400C, 0x00},
	{0x4018, 0x7F},
	{0x401A, 0x37},
	{0x401C, 0x37},
 ...
 * Master Mode
 * Time 11.074ms
 * 1920x1080 2/2-line binning (基于1932x1096裁剪)
 * 2024-09-25 新增配置
 */
static __maybe_unused const struct regval imx415_linear_12bit_1920x1080_1782M_regs[] = {
	{0x3008, 0x7F},
	{0x300A, 0x5B},
	{0x3020, 0x01},
	{0x3021, 0x01},
...
static const struct imx415_mode supported_modes[] = {
	 ...
	 * Register settings from sunnic_IMX415_RegisterSetting_Ver10.0_20240925_No1.ism
	 * VMAX = 0xD4 = 212, HMAX = 0x016D = 365
	 * frame rate = 1 / (VMAX * 1H) = 1 / (212 * HMAX) ≈ 90.011fps
	 */
	{
		...
		.max_fps = {
			.numerator = 10000,
			.denominator = 90011,  // 90.011fps
		},
		.exp_def = 0x00D4 - 0x08,  // VTS - offset, default exposure
		.hts_def = 0x016D * IMX415_4LANES * 2,  // HMAX = 365
		.vts_def = 0x00D4,  // VMAX = 212
		.global_reg_list = imx415_global_12bit_3864x2192_regs,
		.reg_list = imx415_linear_12bit_1920x1080_90fps_1782M_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 5,  // MIPI_FREQ_1782M (index 5 in link_freq_items)
		.bpp = 12,
		.vc[PAD0] = 0,
	 ...
	 * 
	 * 2024-09-25: 新增 1920x1080 @ 90fps 作为默认模式
	 * 使用 2/2-line binning，VMAX=0xD4(212), HMAX=0x16D(365)
	 * 1H period = 365 * 1/74.25MHz = 4.916us
	 * Frame time = 212 * 4.916us = 1.042ms -> 90.01fps
	 */
	{
		.bus_fmt = MEDIA_BUS_FMT_SGBRG12_1X12,
		.width = 1920,
		...
		.reg_list = imx415_linear_12bit_1920x1080_1782M_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 3,
		.bpp = 12,
		.vc[PAD0] = 0,
...
#define DST_WIDTH_1932 1932
#define DST_HEIGHT_1096 1096
				...
				struct v4l2_subdev_selection *sel)
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		...
		} else if (imx415->cur_mode->width == 1920) {
			/* 1920x1080 @ 90fps 模式，直接输出无需裁剪 */
			sel->r.left = 0;
			sel->r.width = DST_WIDTH_1920;
			sel->r.top = 0;
			sel->r.height = DST_HEIGHT_1080;
		} else if (imx415->cur_mode->width == 1932) {
			/* 1932x1096 binning模式，裁剪到1920x1080 */
			sel->r.left = CROP_START(DST_WIDTH_1932, DST_WIDTH_1920);
			sel->r.width = DST_WIDTH_1920;
			sel->r.top = CROP_START(DST_HEIGHT_1096, DST_HEIGHT_1080);
			sel->r.height = DST_HEIGHT_1080;
		} else {
...
static int imx415_initialize_controls(struct imx415 *imx415)
{
	...
	/* Use the maximum frequency in link_freq_items array for max_pixel_rate */
	max_pixel_rate = MIPI_FREQ_1782M / mode->bpp * 2 * lanes;
